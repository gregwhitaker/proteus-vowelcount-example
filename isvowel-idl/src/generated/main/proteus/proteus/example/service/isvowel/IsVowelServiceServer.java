...
public final class IsVowelServiceServer extends io.netifi.proteus.AbstractProteusService {
  ...
  public IsVowelServiceServer(IsVowelService service, io.micrometer.core.instrument.MeterRegistry registry) {
    this.service = service;
    this.isVowel = io.netifi.proteus.metrics.ProteusMetrics.timed(registry, "proteus.server", "namespace", "proteus.example.service.isvowel", "service", "IsVowelService", "method", "isVowel");
  }
    ...
    try {
      ...
      switch(io.netifi.proteus.frames.ProteusMetadata.methodId(metadata)) {
        case IsVowelService.METHOD_IS_VOWEL: {
          ...
          return service.isVowel(proteus.example.service.isvowel.IsVowelRequest.parseFrom(is), metadata).map(serializer).transform(isVowel);
        }
        default: {
